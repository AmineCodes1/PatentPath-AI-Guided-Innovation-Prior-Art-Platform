...
async def get_session_results(
    ...
    risk_filter: Annotated[list[RiskLabel] | None, Query()] = None,
...
async def save_patent_to_project(
    ...
    session = await db.scalar(
        ...
        .where(SearchSession.id == session_id, InnovationProject.user_id == current_user.id)
    )
    if session is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=SESSION_NOT_FOUND_DETAIL)
...
async def get_session_stats(
    ...
    return await get_search_stats(db=db, session_id=session_id, user_id=current_user.id)
